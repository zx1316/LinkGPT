...
interface LinkGPTDao {
    ...
    @Query("SELECT detail_table.name, history_table.output, history_table.time, detail_table.image " +
    ...
    suspend fun getBotList()

    @Insert
    suspend fun insertHistory(botHistoryData: BotHistoryData)

    @Query("SELECT input, output FROM history_table WHERE name = :name and time > ALL(SELECT summaryTime from detail_table WHERE name = :name);")
    suspend fun getValidHistory(name: String)

    @Query("SELECT personality, summary FROM detail_table WHERE name = :name;")
    suspend fun getPersonalityAndSummary(name: String)
